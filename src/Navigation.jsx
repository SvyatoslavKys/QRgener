...
const navItems = [
  ...
  { to: "/scan", labelKey: "nav.scan", icon: "scan" },
  { to: "/generate", labelKey: "nav.create", icon: "qr" },
    ...
    <>
      <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <NavLink to="/" className="group flex items-center gap-3" aria-label={t("brand.homeLabel")}>
            ...
            <span>
              <span className="block text-sm font-extrabold tracking-tight text-slate-950">{t("brand.name")}</span>
          ...
          <nav className="hidden items-center gap-1 md:flex" aria-label={t("brand.mainNavigation")}>
            {navItems.map((item) => (
      ...
      <nav
        ...
        aria-label={t("brand.mobileNavigation")}
      >
        {navItems.map((item) => (
